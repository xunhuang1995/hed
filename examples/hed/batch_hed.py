...
def parse_args():
    ...
    parser.add_argument('--videos_dir', dest='videos_dir', help='directory to store input videos', type=str)
...
videoList = os.listdir(args.videos_dir)
nVideos = len(videoList)
print('#videos = %d' % nVideos)
...
for i in range(nVideos):
    if i % 500 == 0:
        print('processing video %d/%d' % (i, nVideos))
    video_dir = os.path.join(args.videos_dir, videoList[i])
    os.mkdir(os.path.join(args.hed_mat_dir, videoList[i]))
    for j, filename in enumerate(os.listdir(video_dir)):
        im = Image.open(os.path.join(video_dir, filename))
        ...
        sio.savemat(os.path.join(args.hed_mat_dir, videoList[i], name + '.mat'), {'predict':fuse})
